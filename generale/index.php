<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST')
    {
      if(isset($_POST['connexion']))
      {
        ...
        $stmt=$con->prepare("select id_u,username,password from users where username=? and password=? and status=3 and sup=0 LIMIT 1");
        ...
        if ($count>0)
        {
            $_SESSION['Usernameg']=$Username;
            $_SESSION['IDg']=$row['id_u'];
            header('location: dashboard.php');
            echo"ok";
            exit();
        }
...
<form class="login" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        <h4 class="text-center">Général connexion</h4>
